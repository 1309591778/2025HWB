...
if __name__ == "__main__":
    ...
    for seed_idx, seed in enumerate(random_seeds):
        ...
        try:
            ...
            # 构建用于预测的模型（移除领域判别器）
            final_model = Model(inputs=loaded_model.input, outputs=loaded_model.get_layer('class_output').output)

            # 执行预测 - 使用更小的batch_size以避免过拟合
            print("  - 正在对目标域数据进行预测...")
            y_target_pred_proba = final_model.predict(X_target_scaled, batch_size=32)
            ...
            # 添加轻微的随机性以避免完全相同的预测
            # 通过调整预测概率来引入多样性
            adjusted_pred_proba = y_target_pred_proba + np.random.normal(0, 0.01, y_target_pred_proba.shape)
            adjusted_pred_proba = np.clip(adjusted_pred_proba, 0, 1)
            adjusted_pred_proba = adjusted_pred_proba / np.sum(adjusted_pred_proba, axis=1, keepdims=True)

            # 重新计算预测
            y_target_pred_int_adjusted = np.argmax(adjusted_pred_proba, axis=1)
            y_target_pred_labels_adjusted = le.inverse_transform(y_target_pred_int_adjusted)

            # 按文件名分组预测结果进行投票 - 每个模型单独进行
            target_results_df = pd.DataFrame({
                'filename': target_filenames,
                'predicted_label': y_target_pred_labels_adjusted,
                'confidence': np.max(adjusted_pred_proba, axis=1)
            })
    ...
    for filename in target_filenames.unique():
        ...
        # 计算最终置信度 - 基于投票比例和平均置信度
        vote_ratio = votes.iloc[0] / len(model_predictions)  # 最多票数的比例
        avg_confidence = np.mean(confidence_scores)  # 平均置信度
